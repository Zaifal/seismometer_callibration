...
class KalibrasiApp(ctk.CTk):
    ...
    def show_admin_popup(self):
        AdminDataPopup(self, on_save_callback=self.save_admin_data_to_excel)
    ...
    def extract_amplitude_and_update(self):
        ...
        if not self.boundaries:
            messagebox.showerror("Error", "Boundaries belum di-set. Klik 'Set Boundary' terlebih dahulu.")
            return
        ...
        messagebox.showinfo("Sukses", "Ekstraksi amplitudo selesai dan data disimpan ke Excel.")

    # BARU: Fungsi helper terpisah untuk menulis data amplitudo
    ...
    # DIUBAH: Fungsi ini sekarang memanggil helper
    ...
    # DIUBAH: Fungsi ini sekarang lengkap
    def export_certificate(self):
        template_path = "amplitudo_ekstraksi.xlsx"
        if not os.path.exists(template_path):
            messagebox.showerror("Error", f"Template sertifikat tidak ditemukan!\nHarap sediakan file '{template_path}' di folder utama.")
            return
        if not self.latest_admin_data or not self.latest_digitizer_data or not self.latest_amplitude_data:
            messagebox.showwarning("Perhatian", "Harap isi Data Administrasi, pilih Digitizer, dan lakukan Ekstrak Amplitudo terlebih dahulu.")
            return
        ...
        try:
            ...
            # Isi data administrasi
            ws_admin = wb['data_administrasi']
            row = 1
            for key, value in self.latest_admin_data.items():
                ws_admin.cell(row=row, column=1, value=key).font = Font(bold=True)
                ws_admin.cell(row=row, column=2, value=value)
                row += 1

            # Isi data digitizer
            ws_digi = wb['data_digitizer']
            row = 1
            for key, value in self.latest_digitizer_data.items():
                ws_digi.cell(row=row, column=1, value=key).font = Font(bold=True)
                ws_digi.cell(row=row, column=2, value=value)
                row += 1

            # Isi data amplitudo menggunakan fungsi helper
            ws_auto = wb['data_auto']
            self._write_amplitude_data_to_sheet(ws_auto, self.latest_amplitude_data, self.latest_freq_states)
            
            # Simpan file Excel yang sudah terisi penuh
            wb.save(output_excel_path)
            print(f"Data lengkap telah ditulis ke: {output_excel_path}")
